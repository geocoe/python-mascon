...
from fourier_legendre import fourier_legendre
...
def gen_harmonics(data, lon, lat, **kwargs):
    ...
    Example
    ----------
    clm, slm = gen_harmonics(data, lon, lat, LMAX=60, MMAX=None, PLM=0, METHOD='integration')
    ...
    if kwargs['METHOD'].lower() == 'integration':
        clm, slm = integration(dinput, lon, lat, **kwargs)
    elif kwargs['METHOD'].lower() == 'fourier':
        clm, slm = fourier(dinput, lon, lat, **kwargs)
...
def fourier(data, lon, lat, LMAX=60, MMAX=None, PLM=0, **kwargs):
    ...
    for m in range(0, MMAX + 2, 2):
        ...
        clm[l_even, m] = np.dot(theta_cc[m, m_even[:, np.newaxis]].T, temp.T)
        slm[l_even, m] = np.dot(theta_sc[m, m_even[:, np.newaxis]].T, temp.T)
        temp = np.dot(plm[l_odd, m, m_odd[:, np.newaxis]].T, k_odd)
        clm[l_odd, m] = np.dot(theta_cc[m, m_odd[:, np.newaxis]].T, temp.T)
        slm[l_odd, m] = np.dot(theta_sc[m, m_odd[:, np.newaxis]].T, temp.T)
