...
from covariance import disturb_gravity_covariance1
...
def comparision(x,a,time):
    ...
    imgpath = os.path.join(imgpath, f'disturb_gravity_{region_name}_timeseries_{a}')
...
if __name__=="__main__":
    ...
    lmax = 60
    shc, shc_std, gtime, time_cover, lmax, r, gm = read_all_gsm(shc_path)
    cnm_std = shc_std[0, 0, :, :]
    snm_std = shc_std[0, 1, :, :]
    cnm_var = cnm_std ** 2
    snm_var = snm_std ** 2
    H = disturb_gravity_covariance1(cnm_var, snm_var, lmax, lon_d, lat_d)
    U, S, Vh = np.linalg.svd(H)
    print()



    # dis_var_inv = np.linalg.inv(dis_var)
    # # 进行特征值分解
    # eigenvalues, eigenvectors = np.linalg.eig(dis_var)
    # # number = np.sum(eigenvectors[:, 0] ** 2)
    # eigenvalues1 = eigenvalues[:200]
    # eigenvectors1 = eigenvectors[:, :200]
    # # dis_var1 = eigenvectors1 @ np.diag(eigenvalues1) @ eigenvectors1.T
    # # dis_var1_real = dis_var1.real
    # # cond_dis_var = np.linalg.cond(dis_var1_real)
    # # print(f'截断后的重力扰动位协方差条件数为:{cond_dis_var}')
    # eigenvalues2 = eigenvalues1.real
    # eigenvectors2 = eigenvectors1.real
    # P = eigenvectors2 @ np.diag(1 / eigenvalues2) @ eigenvectors2.T
    # Nx = A.T @ P @ A
    # y_all = disgravity[1, :, :]
