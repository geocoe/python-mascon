...
if __name__=='__main__':
    ...
    mascon_path = r'D:\code\point mass\results\mascon\all_TWSA_RMS.nc'
    region_path = r'D:\code\point mass\data\bln\Antarctica.bln'
    ...
    # 1.读取mascon
    location_dataset = xr.open_dataset(location_path)
    mascon_dataset = xr.open_dataset(mascon_path)
    lat = location_dataset['lat'].values
    lon = location_dataset['lon'].values
    mascon_data = mascon_dataset['lwe_thickness'].values
    time = mascon_dataset['time'].values

    # 制作掩膜并生成时间序列
    mask = generate_region_mask(region_path, lon, lat)
    mask = np.squeeze(mask)
    index = np.array(np.where(mask == 1))[:]
    time_series = np.mean(mascon_data[:, index], axis=2)
    ...
    imgpath = os.path.join(imgpath,f'{region_name}_timeseries')
